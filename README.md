...
页面顶部的状态徽标显示当前出口；若两个协议族走不同出口，徽标变红并在提示条中列出各自的实际出口。出口卡片下方显示自动看门狗与链路健康探测的运行状态。

页面自上而下依次是：出口徽标与提示条 → 出口卡片上方的一排动态图标（运行状态总览）→ 出口卡片 → 看门狗状态与操作按钮。只读的读数在上，可改的控件在下。总览区块本身不设页内标题：每个图标自带名称、数值与说明，再加一层 `<h2>` 只会与「网络出口」标题争夺同一视觉层级。
