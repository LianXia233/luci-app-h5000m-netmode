...
| 透明代理兼容 | TUN 只按物理 IPv4 出口归属显示，不作为第三个出口管理，也不重载代理服务 |
...
| 标题右侧 | 在线 / 协商中 / 已断开 / 网线未接 / 未配置 / 外部接管 / 告警 / 离线，外加一枚呼吸圆点 | `wan_*` 或 `modem_*` 的分级链路状态、`split` |
...
| 智能路由 | `active4` / `active6` | 灰色，路径动画运行 |
...
| 带 HomeProxy / sing-box / daed 时持续报「分流」 | v1.6.5 起后端只管理物理 WAN / 5G 模组接口：TUN 会按本轮 IPv4 物理出口归属，不需要 daed 状态文件，也不会重载代理服务。先确认后端含 `is_tunnel_device()`：`grep -c is_tunnel_device /usr/sbin/h5000m-netmode` 应为非 0；再用**带源地址**的查询核对物理出口（见上方自检第 2 条），显示 `dev ethX` 才是真实出口 |
...
不算。`singtun0` 是本地 TUN 隧道而非上行链路。后端按网卡内核类型识别隧道，并把它归属到同一轮状态里已经判定出的 IPv4 物理出口，因此 `active4` 与 `active6` 相同、`split=0`，页面显示「IPv4 与 IPv6 同出口」。只有两个协议族归属到**不同物理上行链路**时才会报分流。
